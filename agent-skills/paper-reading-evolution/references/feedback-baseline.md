...
Status: manually curated from this conversation, in message order where the user changed direction. These are evaluation requirements, not executed tests or a measured benchmark. Full documents and expected evidence are still needed for behavioral evaluation.

## Feedback routing

| ID | User feedback | Correct destination | Forbidden shortcut |
| --- | --- | --- | --- |
| F01 | Earlier: translate using original paragraphs. Later: “翻译的时候还是别分段了”。 | Personal translation-format preference; later explicit request wins in this scope. | Flattening explanations/summaries or retaining two conflicting global instructions. |
| F02 | “对于解释可以适当分段，条理分明”。 | Explanation-specific presentation preference. | Requiring every short answer to have many headings. |
| F03 | Current paper first; “当前文献的参考论文也是重中之重”。 | Evidence retrieval and reference-reading strategy. | Treating a bibliography entry as an inspected source. |
...
| F09 | Style reversions / “我没看到改变”。 | Version visibility and deployment/cache verification in the application. | Reporting success from a build alone. |

## Minimum behavioral evaluation cases

1. A source has multiple paragraphs. The owner's latest translation preference produces faithful continuous translation, while the same source's long explanation remains structured. A different user's formatting is unaffected.
...
11. An explicit lasting preference updates only its user's scope; “这一次请分段” affects only that task. Ambiguous feedback does not silently create a permanent rule.
...
Store actual baseline/candidate outputs, expected supporting passages, tool traces, model settings, first-answer latency, total latency, and token usage with evaluation results. Do not call these cases “passed” until an evaluator runs them. Keep hidden fixtures and authoritative scoring rules outside the evolving skill's editable resources.
